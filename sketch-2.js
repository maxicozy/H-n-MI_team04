...
let maxPixelCount = 3000; // Cap for normalizing volumes (adjust based on your video)
...
function setup() {
  ...
  for (let i = 0; i < videoGridSize * videoGridSize; i++) {
    ...
    let octaveMultiplier = Math.floor(i / 20) * 0.5 + 1;
...
function draw() {
  ...
  for (let y = 0; y < video.height; y++) {
    for (let x = 0; x < video.width; x++) {
      ...
      if (r > threshholds.r && g > threshholds.g && b < threshholds.b) {
        yellowPixelCount++;
